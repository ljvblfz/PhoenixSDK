...
public class DagWalker
{
   ...
   )
   {
      ...
      Phx.Asserts.Assert(dagNode != null,  "DagWalker.Walk, dagNode!=null");
      ...
      foreach (DagEdge edge in dagNode.ChildrenList)
      {
   ...
   protected bool DoPreAction
   {
      set { doPreAction = value; }
   }
   private bool doPreAction;
   ...
   protected bool DoPostAction
   {
      set { doPostAction = value; }
   }
   private bool doPostAction;
...
public class GenCodeWalker : DagWalker
{
   //--------------------------------------------------------------------------
   //
   // Description:
   ...
   {
      GenCodeWalker newWalker = new GenCodeWalker();
   ...
   protected override WalkControl
   PreAction
   (
      DagNode dagNode
   )
   {
      ...
      if (dagNode.IsSimpleUseNode
            && (dagNode.Label.IsImmediateOperand || dagNode.NumRealParent <= 1))
      {
   ...
   )
   {
	 ...
	 // its original dst operand.
      ...
      Phx.IR.Instruction newInstruction = this.GenInstr(null, dagNode);
   ...
   )
   {
      ...
      int numRealParent = this.CountRealParent(dagNode, out goodDstCandidate);
      ...
      else
      {
         ...
         newDestinationOperand = Phx.IR.VariableOperand.NewTemporary(this.functionUnit, dagNode.NewDstOpnd.Type);
      }
   ...
   )
   {
      ...
      Phx.Asserts.Assert(dagNode.TargetNode != null,
         "GenCodeWalker.PostAction, dagNode.TargetNode != null");

#endif

      Phx.IR.Instruction newInstruction = null;
      ...
      else
      {
         ...
         newInstruction = Phx.IR.ValueInstruction.NewUnary(this.functionUnit, Common.Opcode.Assign,
            dagNode.NewDstOpnd, targetNode.NewDstOpnd);
         ...
         if (!newDestinationOperand.Type.IsAssignableFrom(newSourceOperand.Type))
         {
            ...
            newSourceOperand.Field = newSourceOperand.Field.GetField(
               newDestinationOperand.Type, newSourceOperand.Field.BitOffset);
         }
   ...
   )
   {
      ...
      int numRealParent = this.CountRealParent(rightHand, out goodDstNode);
         ...
         && rightHand.Label.Instruction.IsValueInstruction)
      {
         ...
         foreach (DagEdge edge in rightHand.ChildrenList)
         {
      ...
      foreach (DagNode nextSrc in rightHand.SrcList)
      {
      ...
      foreach (Phx.IR.Operand dstOperand in oldInstruction.DestinationOperands)
      {
         if (dstOperand.IsExplicit)
         {
            Phx.IR.Operand newDestinationOperand = null;
            ...
            {
               if (goodDstNode != null && 
                     (goodDstNode.Label.IsTemporary || numRealParent < 2))
               {
               ...
               else
               {
                  ...
                  newDestinationOperand =
                     Phx.IR.VariableOperand.NewTemporary(this.functionUnit, rightHand.NewDstOpnd.Type);
               }
   ...
   (
      DagNode dagNode,
      out DagNode goodDstCandidate
   )
   {
      // Assume we don't have any good dst candidate

      goodDstCandidate = null;

      int numRealParent = dagNode.NumRealParent;

      foreach (DagEdge edge in dagNode.ParentList)
      {
         ...
         if (parent.IsQueriedExprTmp ||
            parent.NumRealParent == 0 && parent.IsKilledBySelf)
         {
         ...
         if (dagNode.IsExprNode)
         {
            ...
            {
               if (goodDstCandidate.IsMemNode && !parent.IsMemNode)
               {
   ...
   private Phx.IR.Instruction baseInstruction;

   // A control enable/disable certain optimizations
