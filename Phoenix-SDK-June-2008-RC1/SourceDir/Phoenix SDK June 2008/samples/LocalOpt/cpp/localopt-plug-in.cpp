...
)
{
   ...
   mirlowerPhase = config->PhaseList->FindByName("MIR Lower");
...
{
   LocalOptPhase ^ phase = gcnew LocalOptPhase();
...
void LocalOptPhase::StaticInitialize ()
{
#if (PHX_DEBUG_CHECKS)
...
)
{
   ...
   Phx::FunctionUnit ^ functionUnit = dynamic_cast<Phx::FunctionUnit ^>(unit);
   ...
   functionUnit->BuildFlowGraph(
      Phx::Graphs::BlockStyle::IgnoreHandlerEdges);
   ...
   for each (Phx::Graphs::BasicBlock ^ block in
...
{
   Optimization ^ newOptimizer = gcnew Optimization();
...
)
{
   ...
   while ((startInstruction != nullptr) && !startInstruction->IsReal
   ...
   while ((endInstruction != nullptr) && (endInstruction != startInstruction)
      && (!endInstruction->IsReal || (endInstruction->IsBranchInstruction 
         && endInstruction->AsBranchInstruction->IsUnconditional)))
   {
   ...
   this->renameMap = gcnew OpndToOpndMap(instructionCount);
...
{
   Phx::IR::Instruction ^ instruction = nullptr;

   for (instruction = startInstruction;
        instruction != endInstruction->Next;
        instruction = instruction->Next)
   {
      ...
      if (instruction->IsCopy)
      {
         ...
         newTargetNode = theDag->FindOrCreateNode(instruction->SourceOperand,
            DagNodeKind::Use);
      ...
      else
      {
         ...
         newTargetNode = theDag->FindOrCreateNode(instruction->DestinationOperand,
            DagNodeKind::Expression);
      }
...
)
{
   ...
   switch (Phx::Common::Opcode::GetIndex(opcode))
   {
      ...
      // If there is any other case, put them here. 
...
)
{
   ...
   Phx::Asserts::Assert(this->renameMap != nullptr,
   ...
   GenCodeWalker ^ walker = GenCodeWalker::New(
      this->functionUnit, theDag, startInstruction);
   ...
   for each (DagNode ^ rootNode in theDag->RootList)
   {
   ...
   Phx::IR::Instruction::Unlink(Phx::IR::InstructionRange(startInstruction, endInstruction));
...
)
{
   ...
   if (instruction->LastExplicitSourceOperand != nullptr)
   {
      ...
      while (srcOperand != nullptr)
      {
         if (srcOperand->IsDataflow)
         {
            ...
            Phx::IR::Operand ^ newSourceOperand =
               this->renameMap->Lookup(OpndKey::New(srcOperand,
                     DAG::GetHashCode(srcOperand)));

            if (newSourceOperand != nullptr)
            {
   ...
   for each (Phx::IR::Operand ^ dstOperand in instruction->DestinationOperands)
   {
      ...
      OpndKeyList ^ toDelete = gcnew OpndKeyList();

      for each (OpndKey ^ key in this->renameMap->Keys)
      {
      ...
      for each (OpndKey ^ key in toDelete)
      {
...
)
{
   ...
   {
      Phx::IR::Operand ^ newBaseOperand =
         this->renameMap->Lookup(OpndKey::New(baseOperand,
               DAG::GetHashCode(baseOperand)));

      if (newBaseOperand != nullptr)
      {
   ...
   if (indexOperand != nullptr)
   {
      // Update the IndexOperand if it is resolved to something else

      Phx::IR::Operand ^ newIndexOperand =
         this->renameMap->Lookup(OpndKey::New(indexOperand,
               DAG::GetHashCode(indexOperand)));

      if ((newIndexOperand != nullptr)
   ...
   if (segmentOperand != nullptr)
   {
      // Update the SegmentOperand if it is resolved to something else

      Phx::IR::Operand ^ newSegmentOperand = this->renameMap->Lookup(OpndKey::New(segmentOperand,
            DAG::GetHashCode(segmentOperand)));

      if ((newSegmentOperand != nullptr) && !this->aliasInfo->MustExactlyOverlap(segmentOperand,
            newSegmentOperand))
      {
...
)
{
      ...
      && instruction->Next->IsBranchInstruction && instruction->Next->AsBranchInstruction->IsConditional)
   {
      ...
      foldedValue = Phx::Constant::Info::EvaluateBinary(&result, compareInstruction->Opcode,
         compareInstruction->ConditionCode, compareInstruction->SourceOperand1->Type, srcValue1, srcValue2);

      if ((foldedValue != nullptr) && !result.HasRestrictions)
      {
         // Folding succeeds
         ...
         for each (Phx::IR::Operand ^ srcOperand1 in branchInstruction->SourceOperands)
         {
